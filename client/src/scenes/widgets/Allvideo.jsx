...
const Allvideo = () => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        const response = await axios.get("http://192.168.60.254:3001/videos");
  ...
  const handleWatchVideo = async (video) => {
    try {
      await axios.put(`http://192.168.60.254:3001/videos/${video._id}/view`);
            ...
            <Grid item xs={12} sm={6} md={4} key={video._id}>
              <Card elevation={3}>
                <CardContent>
                  ...
                  <Box
                    component="img"
                    src={`http://192.168.60.254:3001/assets/${video.picturePath}`}
                    alt="video thumbnail"
                    sx={{
                      display: "block",
                      maxWidth: "100%",
